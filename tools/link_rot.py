...
⚠️ Claude Code 웹 컨테이너에서는 동작하지 않는다(프록시가 blogspot 차단).
   GitHub Actions 러너에서 실행한다.
    ...
    try:
        ...
        if e.code in (403, 405, 400, 501, 500):
            try:
                ...
                return "error", None, url, type(e2).__name__
        else:
        ...
        try:
            ...
            return "error", None, url, "%s: %s" % (type(e2).__name__, e2)
...
def main():
    ...
    ap.add_argument("--skip-external", action="store_true",
                    help="공식·내부·이미지만 검사하고 그 외 외부 링크는 건너뛴다")
    args = ap.parse_args()
    ...
    ok = len(buckets["ok"])
    print("-" * 66)
    print("정상 %d · 이전 %d · 죽음 %d · 차단 %d · 서버오류 %d · 확인실패 %d"
          % (ok, len(moved), len(dead), len(blocked), len(server), len(errs)))
    if not dead:
        print("✅ 확실히 죽은 링크는 없다.")
