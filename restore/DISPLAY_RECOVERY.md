...
## 0단계 — BIOS 화면이 외부로 안 나오는 것은 정상이다

**BIOS/POST 화면이 외부 모니터에 안 나온다고 해서 포트가 죽은 게 아니다.**
대부분의 노트북은 POST 단계에서 외부 출력을 초기화하지 않는다:

- **USB-C / Thunderbolt (DP alt-mode)** — OS 드라이버가 올라와야 링크가 맺힌다. POST 출력 없음이 정상.
- **하이브리드 그래픽에서 HDMI가 dGPU에 배선된 경우** — dGPU는 드라이버가 초기화하므로 POST 때 죽어 있다.
- **HDMI가 iGPU에 직결된 경우** — 이때만 POST 화면이 나온다.

즉 "BIOS는 안 나오는데 Windows에서는 나온다"가 **정상 조합**이다.
따라서 POST 출력이 없어도 A경로를 반드시 먼저 시도해야 한다.

해볼 것: **모니터를 부팅 전에 미리 연결해 둔다** (드라이버 초기화 시점에 감지되도록),
그리고 **HDMI ↔ USB-C 포트를 바꿔서** 각각 시도한다. 내부 배선 경로가 달라 한쪽만 살아 있을 수 있다.

---

## A경로 — 안전모드 탈출 후 DisplaySwitch (권장, 가장 빠름)

내장 패널이 안 보이니 **전부 블라인드 타이핑**이다. 각 단계마다 넉넉히 기다려라.

### A-0. 확인된 원인 (2026-07-26)

**안전부팅이 명령으로 설정되었다.** 터미널에서 PC 문제를 조치하던 중 안전부팅이
실행되어 플래그가 BCD에 기록된 상태다. 이 플래그는 **스스로 풀리지 않으므로 재부팅을
반복해도 계속 안전모드로 들어간다.**

그리고 **안전부팅 이전에는 외부 모니터로 정상 작업이 되고 있었다.** 이 사실이 중요하다:

- Windows는 이 외부 모니터를 **이미 정상적으로 인식·구동한다.**
- 따라서 모니터 감지 여부를 진단할 필요가 없고, 내장 패널을 비활성화할 필요도 없다.
- **안전부팅 플래그만 지우면 원래 상태로 복귀한다.** 할 일은 그것 하나다.

### A-1. 안전모드 플래그 제거 — 키보드만으로 (준비물 없음)

**⚠️ 안전모드에서는 시작 메뉴 검색이 동작하지 않는다** (Windows Search 서비스가 로드되지
않음). 그래서 "시작 → cmd 입력" 방식은 안전모드에서 실패한다.
대신 `Win + X` 전원 사용자 메뉴를 쓴다 — 검색에 의존하지 않는다.

1. 부팅 완료까지 **3분** 기다린다.
2. 비밀번호 입력 + `Enter` → 다시 **1분** 대기 (바탕화면 로딩).
3. `Win + X` → **2초 대기** → `A` 키
   (Win10 = "Windows PowerShell(관리자)", Win11 = "터미널(관리자)")
4. UAC 창 → **3초 대기** → `Alt + Y`
5. **3초 대기** 후 아래를 타이핑하고 `Enter`:
   ```
   bcdedit /deletevalue {current} safeboot
   ```
6. 이어서 타이핑하고 `Enter` (부팅 항목이 다를 경우를 대비한 이중 안전장치):
   ```
...
재부팅되면 정상 모드로 올라오고, **외부 모니터가 예전처럼 켜진다.**
`DisplaySwitch` 를 따로 실행할 필요도 없다 — 원래 되던 구성이므로 그대로 복원된다.

### A-2. 정상 부팅 후 출력 전환

정상 모드로 부팅되면 실제 GPU 드라이버가 올라오므로 외부 출력이 가능해진다.

**`Win + P` 로 화살표를 세는 것보다 확실한 방법이 있다.** `Win + P` 패널을 실제로 구동하는
실행파일을 직접 호출하면 된다 — 몇 번 눌렀는지 셀 필요가 없다:

1. 로그인 (비밀번호 + `Enter`) → **1분** 대기
2. `Win + R` → 아래를 타이핑 → `Enter`
   ```
   DisplaySwitch.exe /external
   ```

이게 "두 번째 화면만"과 정확히 같은 동작이다. 관리자 권한도 필요 없다.
다른 인자도 쓸 수 있다:

| 명령 | 효과 |
|---|---|
| `DisplaySwitch.exe /external` | 두 번째 화면만 ← **내장 패널 고장 시 정답** |
| `DisplaySwitch.exe /clone` | 복제 (양쪽 같은 화면) |
| `DisplaySwitch.exe /extend` | 확장 |
| `DisplaySwitch.exe /internal` | PC 화면만 (되돌리기) |

`/external` 이 안 먹으면 `/clone` 도 시도하라. 확장/외부전용이 EDID 협상에 실패하는
경우에도 복제는 붙는 사례가 있다.

3. 그래도 안 되면 `Ctrl + Win + Shift + B` (그래픽 드라이버 리셋) → 30초 대기 → 2번 재시도.
4. 대체 수단으로 `Win + P` → 1초 대기 → `↓` `↓` `↓` → `Enter`
   (목록 순서: [PC 화면만 / 복제 / 확장 / **두 번째 화면만**])

> `Win + P` 와 `DisplaySwitch` 모두 로그인 **전**에는 동작하지 않는다. 반드시 로그인 후에 하라.

> 내장 패널이 물리적으로 깨졌어도 **전기적으로는 살아 있으면** Windows는 그것을 계속
> 주 디스플레이로 인식한다. 그래서 자동 전환이 안 되고 위 명령이 필요하다.
